...
var Title = function ( $glob ) { 
 ...
 self.awake = function ( list ){
   ...
   self.buffer = self.glob.width * .03;
 ...
 self.assetLoaderComplete = function(){
	...
	self.logo = new PIXI.Sprite(texture);
	self.display.addChild( self.logo );

	texture = PIXI.Texture.fromImage( self.graphics[1] );
	self.sparkle = new PIXI.Sprite( texture );
	self.logo.addChild( self.sparkle );
 ...
 self.sparkle1 = function(){
	...
	self.sparkle.position.x = 410;
	self.sparkle.position.y = -0;
 	setTimeout( self.sparkle2, 50);
 }

 self.sparkle2 = function(){
 	self.sparkle.position.x = 420;
	self.sparkle.position.y = -10;
 	setTimeout( self.sparkle3, 54);
 }

 self.sparkle3 = function(){
 	self.sparkle.position.x = 400;
	self.sparkle.position.y = 5;
 	setTimeout( self.sparkleOff, 30);
 }
